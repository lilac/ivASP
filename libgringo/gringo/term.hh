...
struct Term;
typedef std::unique_ptr<Term> UTerm;
class Defines {
public:
    typedef std::unordered_map<FWString, std::tuple<bool, Location, UTerm>> DefMap;
    ...
    void init();
    bool empty() const;
    void apply(Value x, Value &retVal, UTerm &retTerm);
...
struct GLinearTerm;
typedef std::unique_ptr<GTerm> UGTerm;
struct GTerm : Printable, Hashable, Comparable<GTerm> {
    typedef std::pair<bool, Value> EvalResult;
...
typedef std::vector<UGTerm> UGTermVec;
typedef std::shared_ptr<GRef> SGRef;

// }}}
// {{{ declaration of Term

struct VarTerm;
typedef std::vector<UTerm> UTermVec;
typedef std::vector<UTermVec> UTermVecVec;
typedef std::vector<std::reference_wrapper<VarTerm>> VarTermVec;
typedef std::vector<std::pair<VarTerm*,bool>> VarTermBoundVec;

struct Term : public Printable, public Hashable, public Locatable, public Comparable<Term>, public Clonable<Term> {
	typedef std::shared_ptr<Value> SVal;
	typedef std::unordered_set<FWString> VarSet;
    typedef std::unordered_map<FWString, std::pair<FWString, SVal>> RenameMap;
    typedef std::unordered_map<Term*, SGRef, value_hash<Term*>, value_equal_to<Term*>> ReferenceMap;

    //! Type that stores for each rewritten DotsTerm the associated variable and the lower and upper bound.
    typedef std::vector<std::tuple<UTerm, UTerm, UTerm>> DotsMap;
    ...
    typedef std::vector<std::unordered_map<UTerm, UTerm, value_hash<UTerm>, value_equal_to<UTerm>>> ArithmeticsMap;
    ...
    virtual SimplifyRet simplify(DotsMap &dots, unsigned &auxNum, bool positional, bool arithmetic) = 0;
    //! Return value of Term::project.
    typedef std::tuple<UTerm, UTerm, UTerm> ProjectRet;
    ...
    virtual ProjectRet project(unsigned &auxNum) = 0;
    ...
    virtual UTerm replace(Defines &defs, bool replace = true) = 0;
...
struct LinearTerm : public Term {
    typedef std::unique_ptr<VarTerm> UVarTerm;
