...
struct FullIndex : IndexUpdater {
    ...
    FullIndex(exports_type &exports, UTerm &&repr);
...
struct AbstractDomain : Domain {
    ...
    void init();
    element_type &reserve(Value x);
    bind_index_type &add(SValVec &&bound, UTerm &&repr);
    full_index_type &add(UTerm &&repr);
...
using PredDomMap = unique_list<FWSignature, PredicateDomain>;
...
template <class Element>
FullIndex<Element>::FullIndex(exports_type &exports, UTerm &&repr) 
    : repr(std::move(repr))
    , exports(exports) { }
template <class Element>
...
template <class Element>
bool FullIndex<Element>::operator==(FullIndex const &x) const { return *repr == *x.repr; }
template <class Element>
size_t FullIndex<Element>::hash() const                       { return repr->hash(); }
template <class Element>
...
template <class Element>
typename AbstractDomain<Element>::full_index_type &AbstractDomain<Element>::add(UTerm &&repr) {
    auto ret(fullIndices.emplace(exports, std::move(repr)));
