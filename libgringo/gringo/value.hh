...
// TODO: provide a template for flyweights
...
struct Value {
    enum Type : unsigned { INF, NUM, ID, STRING, TUPLE, FUNC, SPECIAL, SUP };
    struct POD;
...
inline size_t Signature::hash() const {
    return std::hash<uint64_t>()(uint64_t(name_.uid()) + (uint64_t(length_) << 32));
}
...
inline Value::Value(FWValVec args)
    : Value(TUPLE | (args.length() << typeBits), args.offset()) {
...
inline Value::Value(FWString name, FWValVec args)
    : Value(FUNC | (FWSignature(name, args.length()).uid() << typeBits),  args.offset()) {
    assert(args.length() > 0);
}
...
inline size_t Value::hash() const {
    return std::hash<uint64_t>()(uint64_t(type_) + (uint64_t(value_) << 32));
}
...
inline bool Value::less(Value const &other) const {
    ...
    switch(type()) {
        ...
        case TUPLE:     {
            ...
            if (aa.length() != ab.length()) { return aa.length() < ab.length(); }
...
inline FWValVec Value::args() const {
    if (type() == TUPLE) { return FWValVec(type_ >> typeBits, value_); }
    else {
        assert(type() == FUNC);
...
struct hash<Gringo::Signature> {
    size_t operator()(Signature const &sig) const { return sig.hash(); }
};

template<>
struct hash<Gringo::Value> {
    size_t operator()(Value const &val) const { return val.hash(); }
};
