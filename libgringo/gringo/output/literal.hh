...
#include <gringo/output/interfaces.hh>
...
struct AuxLiteral : Literal {
    ...
    virtual ULit toLparse(unsigned &, StmHandler const &);
    virtual void printPlain(std::ostream &out) const;
    virtual bool isIncomplete() const;
    virtual int lparseUid(LparseOutputter &out) const;
    virtual void makeEqual(ULit &&lit, unsigned &auxAtoms, StmHandler const &x) const;
...
using BdAggrElemSet = unique_list<FWValVec, std::vector<ULitVec>>;
...
struct BodyAggregate : Literal {
    ...
    virtual ULit toLparse(unsigned &auxAtoms, StmHandler const &x);
    virtual void makeEqual(ULit &&lit, unsigned &auxAtoms, StmHandler const &x) const;
    virtual int lparseUid(LparseOutputter &out) const;
...
struct ConjunctionState {
    ...
    using Elem         = std::pair<PredicateDomain::element_type*, ULitVec>;
    using ElemSet      = std::vector<Elem>;
