...
template <class Element>
inline UIdx make_binder(AbstractDomain<Element> &domain, NAF naf, Term const &repr, typename AbstractDomain<Element>::element_type *&elem, BinderType type, bool recursive, Term::VarSet &bound) {
    using DomainType          = AbstractDomain<Element>;
    ...
    if (naf == NAF::POS) {
        ...
        else { return make_unique<PredicateMatcher>(elem, domain, repr, RECNAF::POS); }
    }
    else { return make_unique<PredicateMatcher>(elem, domain, repr, recnaf(naf, recursive)); }
}
