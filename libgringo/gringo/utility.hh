...
// TODO: remove or change this one
template <class T, class U>
void print_comma(std::ostream &out, T const &begin, T const &end, U const &f, const char *sep);
...
inline std::pair<T, U> clone<std::pair<T, U>>::operator()(std::pair<T, U> const &x) const {
    return make_pair(get_clone(x.first), get_clone(x.second));
}
