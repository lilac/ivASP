...
template <class T>
unsigned FlyweightVec<T>::length() const {
    return length_;
}
...
template <class T, class C>
struct hash<Flyweight<T, C>> {
    size_t operator()(Flyweight<T, C> const &f) const {
        return f.uid();
    }
};

template <class T>
struct hash<FlyweightVec<T>> {
    size_t operator()(FlyweightVec<T> const &f) const {
        size_t seed = typeid(FlyweightVec<T>).hash_code();
        for (auto &x : f) { hash_combine(seed, std::hash<T>()(x)); }
        return seed;
    }
};
