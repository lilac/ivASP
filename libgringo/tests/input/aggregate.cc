...
void TestAggregate::test_unpool() {
    ...
    // disjunction
    CPPUNIT_ASSERT_EQUAL(std::string("[p:r,s;q:r,s;t:]"), to_string(unpool(hdaggr(condvec6()))));
}

void TestAggregate::test_simplify() {
    // body tuple aggregate
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0>#sum{#X1:#X2,#range(#X1,3,4),#range(#X2,5,6)},[(#X0,1,2)])"), simplify(bdaggr(NAF::POS, AggregateFunction::SUM, boundvec(Relation::LT, dterm(1,2)), bdelemvec(termvec(dterm(3,4)), litvec(dlit(5,6))))));
    // body lit aggregate
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0>#sum{#X1:#X2,#range(#X1,3,4),#range(#X2,5,6)},[(#X0,1,2)])"), simplify(bdaggr(NAF::POS, AggregateFunction::SUM, boundvec(Relation::LT, dterm(1,2)), condlitvec(dlit(3,4), litvec(dlit(5,6))))));
    // conjunction
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0:#X1,#range(#X1,3,4),[(#X0,1,2)])"), simplify(bdaggr(dlit(1,2), litvec(dlit(3,4)))));
    // head tuple aggregate
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0>#sum{#X1:#X2:#X3,#range(#X1,3,4),#range(#X2,5,6),#range(#X3,7,8)},[(#X0,1,2)])"), simplify(hdaggr(AggregateFunction::SUM, boundvec(Relation::LT, dterm(1,2)), hdelemvec(termvec(dterm(3,4)), dlit(5,6), litvec(dlit(7,8))))));
    // head lit aggregate
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0>#sum{#X1:#X2,#range(#X1,3,4),#range(#X2,5,6)},[(#X0,1,2)])"), simplify(hdaggr(AggregateFunction::SUM, boundvec(Relation::LT, dterm(1,2)), condlitvec(dlit(3,4), litvec(dlit(5,6))))));
    // disjunction
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0:#X1,#range(#X1,3,4),[(#X0,1,2)])"), simplify(hdaggr(condlitvec(dlit(1,2), litvec(dlit(3,4))))));
}

void TestAggregate::test_rewriteArithmetics() {
    // body tuple aggregate
    CPPUNIT_ASSERT_EQUAL(std::string("((A+B)>#sum{(C+D):#X0,#X0=(E+F)},[{}],[])"), rewrite(bdaggr(NAF::POS, AggregateFunction::SUM, boundvec(Relation::LT, aterm("A","B")), bdelemvec(termvec(aterm("C","D")), litvec(alit("E","F"))))));
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0=#sum{(C+D):#X0},[{(A+B):#X0}],[])"), rewrite(bdaggr(NAF::POS, AggregateFunction::SUM, boundvec(Relation::ASSIGN, aterm("A","B")), bdelemvec(termvec(aterm("C","D")), litvec(alit("A","B"))))));
    // body lit aggregate
    CPPUNIT_ASSERT_EQUAL(std::string("((A+B)>#sum{(C+D):#X0,#X0=(E+F)},[{}],[])"), rewrite(bdaggr(NAF::POS, AggregateFunction::SUM, boundvec(Relation::LT, aterm("A","B")), condlitvec(alit("C","D"), litvec(alit("E","F"))))));
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0=#sum{(C+D):#X0},[{(A+B):#X0}],[])"), rewrite(bdaggr(NAF::POS, AggregateFunction::SUM, boundvec(Relation::ASSIGN, aterm("A","B")), condlitvec(alit("C","D"), litvec(alit("A","B"))))));
    // conjunction
    CPPUNIT_ASSERT_EQUAL(std::string("((C+D):#X0,#X0=(E+F),[{}],[])"), rewrite(bdaggr(alit("C","D"), litvec(alit("E","F")))));
    // head tuple aggregate
    CPPUNIT_ASSERT_EQUAL(std::string("((A+B)>#sum{(C+D):(E+F):#X0,#X0=(G+H)},[{}])"), rewrite(hdaggr(AggregateFunction::SUM, boundvec(Relation::LT, aterm("A","B")), hdelemvec(termvec(aterm("C","D")), alit("E", "F"), litvec(alit("G","H"))))));
    // head lit aggregate
    CPPUNIT_ASSERT_EQUAL(std::string("((A+B)>#sum{(C+D):#X0,#X0=(E+F)},[{}])"), rewrite(hdaggr(AggregateFunction::SUM, boundvec(Relation::LT, aterm("A","B")), condlitvec(alit("C","D"), litvec(alit("E","F"))))));
    // disjunction
    CPPUNIT_ASSERT_EQUAL(std::string("((C+D):#X0,#X0=(E+F),[{}])"), rewrite(hdaggr(condlitvec(alit("C","D"), litvec(alit("E","F"))))));
}
