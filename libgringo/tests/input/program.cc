...
std::pair<Program, Defines> parse(std::string const &str) {
    ...
    NongroundProgramBuilder pb{ ret.first, out, ret.second };
...
void TestProgram::test_rewrite() {
    ...
    CPPUNIT_ASSERT_EQUAL(std::string("p((X+Y)):-q(#X0);#X0=(X+Y)."), rewrite(parse("p(X+Y):-q(X+Y).")));
    CPPUNIT_ASSERT_EQUAL(std::string("(X+Y)<=#count{(X+Y):q((X+Y)):r(#X0),s(#X1),#X1=(A+B)}:-t(#X0);1<=#count{(X+Y):u(#X0),v(#X2),#X2=(A+B)};#X0=(X+Y)."), rewrite(parse("X+Y#count{X+Y:q(X+Y):r(X+Y),s(A+B)}:-t(X+Y),1#count{X+Y:u(X+Y),v(A+B)}.")));
    CPPUNIT_ASSERT_EQUAL(std::string("p(#X0):-q(#X1);#range(#X0,X,Y);#range(#X1,A,B)."), rewrite(parse("p(X..Y):-q(A..B).")));
    CPPUNIT_ASSERT_EQUAL(std::string("p(1):-q.p(2):-q.p(3):-q.p:-q."), rewrite(parse("p(1;2;3;):-q.")));
    CPPUNIT_ASSERT_EQUAL(std::string("p(Z):-#s1(A,B,X,Y);Z=#count{1,0,r(X,Y):r(X,Y)}.#s0(A,B,Y):-p(A,B);Y=#count{1,0,q(B):q(B)}.#s1(A,B,X,Y):-#s0(A,B,Y);X=#count{1,0,q(A):q(A)}."), rewrite(parse("p(Z):-p(A,B),X={q(A)},Y={q(B)},Z={r(X,Y)}.")));
    CPPUNIT_ASSERT_EQUAL(std::string("p(Z):-#s0(Z);Z>0.#s0(Z):-Z=#count{1,0,p(X):p(X)}."), rewrite(parse("p(Z):-Z={p(X)},Z>0.")));
}
...
void TestProgram::test_projection() {
    CPPUNIT_ASSERT_EQUAL(std::string("x:-#p0.#p0:-p(#P0)."), rewrite(parse("x:-p(_).")));
    CPPUNIT_ASSERT_EQUAL(std::string("x:-#p0(X,f).#p0(#X1,f):-p(#P0,#X1,f)."), rewrite(parse("x:-p(_,X,f).")));
    CPPUNIT_ASSERT_EQUAL(std::string("x:-#p0.y:-#p0.#p0:-p(#P0)."), rewrite(parse("x:-p(_).y:-p(_).")));
    CPPUNIT_ASSERT_EQUAL(std::string("x:-#p0(f(1),2).y:-#p1(f(X),g(X)).#p0(f(1),2):-p(f(1,#P0),2).#p1(f(#X0),g(#X2)):-p(f(#X0,#P1),g(#X2))."), rewrite(parse("x:-p(f(1,_),2).y:-p(f(X,_),g(X)).")));
    CPPUNIT_ASSERT_EQUAL(std::string("x:-#not #p0.#p0:-p(#P0)."), rewrite(parse("x:-not p(_).")));
    CPPUNIT_ASSERT_EQUAL(std::string("x:-#p0(1).y:-#p1(1).#p0(1):-p(#P0,1).#p1(1):-p(1,#P0)."), rewrite(parse("x:-p(_,1).y:-p(1,_).")));
    CPPUNIT_ASSERT_EQUAL(std::string("x:-1<=#count{1,0,y:#p0,y}.#p0:-p(#P0)."), rewrite(parse("x:-1{y:p(_)}.")));
}
