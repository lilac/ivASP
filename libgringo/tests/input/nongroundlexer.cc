...
void TestNongroundLexer::test_lexer() {
    ...
    Program prg;
    Defines defs;
    NongroundProgramBuilder pb(prg, out, defs);
    ...
    NonGroundGrammar::location loc;
    NonGroundGrammar::parser::semantic_type val;

    CPPUNIT_ASSERT_EQUAL(int(NonGroundGrammar::parser::token::LUA), ngp.lex(&val, loc));
    ...
    CPPUNIT_ASSERT_EQUAL(5u, loc.begin.line);
    CPPUNIT_ASSERT_EQUAL(23u, loc.begin.column);
