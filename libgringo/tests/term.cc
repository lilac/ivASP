...
std::pair<UTerm, Term::DotsMap> rewriteDots(UTerm &&x) {
    Term::DotsMap dots;
    unsigned int n = 0;
    x->simplify(dots, n, true, false).update(x);
    return std::make_pair(std::move(x), std::move(dots));
}
...
std::vector<std::tuple<UTerm, Term::DotsMap, Term::ArithmeticsMap>> rewrite(UTerm &&x) {
    ...
    for(auto &term : unpool(x)) {
        ...
        arith.emplace_back();
        term->simplify(dots, auxNum, true, false).update(term);
...
UTerm bindVars(UTerm &&x) {
	...
	Term::VarSet set;
	arith.emplace_back();
	x->simplify(dots, auxNum, true, false).update(x);
...
void TestTerm::test_rewriteArithmetics() {
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0,[{(X\\Y):#X0}])"), rewriteArithmetics(binop(BinOp::MOD, var("X"), var("Y"))));
    CPPUNIT_ASSERT_EQUAL(std::string("(f(#X0,1),[{(g(X)\\Y):#X0}])"), rewriteArithmetics(fun("f", binop(BinOp::MOD, fun("g", var("X")), var("Y")), val(1))));
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0,[{@f((@g(X)\\Y)):#X0}])"), rewriteArithmetics(lua("f", binop(BinOp::MOD, lua("g", var("X")), var("Y")))));
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0,[{|(X\\Y)|:#X0}])"), rewriteArithmetics(unop(UnOp::ABS, binop(BinOp::MOD, var("X"), var("Y")))));
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0,[{(-(X\\Y)):#X0}])"), rewriteArithmetics(unop(UnOp::NEG, binop(BinOp::MOD, var("X"), var("Y")))));
    CPPUNIT_ASSERT_EQUAL(std::string("(f(#X0,#X0),[{(X\\Y):#X0}])"), rewriteArithmetics(fun("f", binop(BinOp::MOD, var("X"), var("Y")), binop(BinOp::MOD, var("X"), var("Y")))));
}

void TestTerm::test_rewriteDots() {
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0,[(#X0,X,Y)])"), to_string(rewriteDots(dots(var("X"), var("Y")))));
    CPPUNIT_ASSERT_EQUAL(std::string("(#X0,[(#X0,1,2)])"), to_string(rewriteDots(dots(val(1), val(2)))));
    CPPUNIT_ASSERT_EQUAL(std::string("(f(#X0,3),[(#X0,g(1),2)])"), to_string(rewriteDots(fun("f", dots(fun("g", val(1)), val(2)), val(3)))));
    CPPUNIT_ASSERT_EQUAL(std::string("(@f(#X0,3),[(#X0,@g(X),2)])"), to_string(rewriteDots(lua("f", dots(lua("g", var("X")), val(2)), val(3)))));
    CPPUNIT_ASSERT_EQUAL(std::string("((1*#X0+4),[(#X0,3,3)])"), to_string(rewriteDots(binop(BinOp::ADD, dots(binop(BinOp::ADD, val(1), val(2)), val(3)), val(4)))));
    CPPUNIT_ASSERT_EQUAL(std::string("(|#X0|,[(#X0,1,2)])"), to_string(rewriteDots(unop(UnOp::ABS, dots(unop(UnOp::ABS, val(1)), val(2))))));
}
...
void TestTerm::test_rewrite() {
    CPPUNIT_ASSERT_EQUAL(std::string("[(f((1*#X1+1)),[(#X0,2,3),(#X1,1,#X0)],[{}]),(f((1*#X3+1)),[(#X2,2,4),(#X3,1,#X2)],[{}]),(f((-1*X+1)),[],[{}])]"), to_string(rewrite(fun("f", pool(binop(BinOp::ADD, dots(val(1), dots(val(2), pool(val(3), val(4)))), val(1)), binop(BinOp::SUB, val(1), var("X")))))));
}
...
void TestTerm::test_project() {
    CPPUNIT_ASSERT_EQUAL(std::string("(p,p,p(#P0))"), to_string(rewriteProject(fun("p", var("_")))));
    CPPUNIT_ASSERT_EQUAL(std::string("(p(X),p(#X0),p(#X0,#P1))"), to_string(rewriteProject(fun("p", var("X"), var("_")))));
    CPPUNIT_ASSERT_EQUAL(std::string("(p(g),p(g),p(g(#P0)))"), to_string(rewriteProject(fun("p", fun("g", var("_"))))));
    CPPUNIT_ASSERT_EQUAL(std::string("(p(f(X),g),p(f(#X1),g),p(#P0,f(#X1,#P2),g(#P3)))"), to_string(rewriteProject(fun("p", var("_"), fun("f", var("X"), var("_")), fun("g", var("_"))))));
    CPPUNIT_ASSERT_EQUAL(std::string("(p(f(h,(1*X+2)),g),p(f(h,#X2),g),p(#P0,f(h(#P1),#X2,#P3),g(#P4)))"), to_string(rewriteProject(fun("p", var("_"), fun("f", fun("h", var("_")), binop(BinOp::ADD, var("X"), val(2)), var("_")), fun("g", var("_"))))));
    CPPUNIT_ASSERT_EQUAL(std::string("((1*#X0+1),#X1,#X1)"), to_string(rewriteProject(binop(BinOp::ADD, var("_"), val(1)))));
}
