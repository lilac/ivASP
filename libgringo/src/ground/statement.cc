...
UIdx AssignmentAggregateLiteral::index(BinderType type, Term::VarSet &bound) {
    return make_binder(*dom, NAF::POS, *dom->repr, gLit.repr, type, isRecursive(), bound);
}
UIdx BodyAggregateLiteral::index(BinderType type, Term::VarSet &bound) {
    return make_binder(*dom, gLit.naf, *dom->repr, gLit.repr, type, isRecursive(), bound);
}
UIdx ConjunctionLiteral::index(BinderType type, Term::VarSet &bound) {
    return make_binder(*dom, NAF::POS, *dom->rep, gLit.repr, type, isRecursive(), bound);
}
UIdx HeadAggregateLiteral::index(BinderType type, Term::VarSet &bound) {
    return make_binder(*dom, NAF::POS, *dom->repr, gResult, type, isRecursive(), bound);
}
UIdx DisjunctionLiteral::index(BinderType type, Term::VarSet &bound) {
    return make_binder(*dom, NAF::POS, *dom->repr, gResult, type, isRecursive(), bound);
}
...
Literal::Score BodyAggregateLiteral::score(Term::VarSet const &bound)       { return gLit.naf == NAF::POS ? dom->repr->estimate(dom->exports.size(), bound) : 0; }
Literal::Score AssignmentAggregateLiteral::score(Term::VarSet const &bound) { return dom->repr->estimate(dom->exports.size(), bound); }
Literal::Score ConjunctionLiteral::score(Term::VarSet const &bound)         { return dom->rep->estimate(dom->exports.size(), bound); }
Literal::Score HeadAggregateLiteral::score(Term::VarSet const &bound)       { return dom->repr->estimate(dom->exports.size(), bound); }
Literal::Score DisjunctionLiteral::score(Term::VarSet const &bound)         { return dom->repr->estimate(dom->exports.size(), bound); }
...
void WeakConstraint::report(Output::OutputBase &out) {
    ...
    if (out.tempVals.front().type() == Value::NUM) {
        ...
        out.minimize.elems.emplace_back(
...
void BodyAggregateAccumulate::printHead(std::ostream &out) const {
    ...
    print_comma(out, tuple.begin(), tuple.end(), f, ",");
    out << ":" << *defines.repr;
}
...
Rule::Rule(PredicateDomain *domain, UTerm &&repr, ULitVec &&lits)
    : domain(domain)
...
bool Rule::isNormal() const                          { return bool(defines); }
...
InstVec linearize(bool positive, SolutionCallback &cb, Term::VarSet &&important, ULitVec &lits, ULitVec *aux = nullptr) {
    InstVec insts;
    ...
    // TODO: mergeable with loop below?
    for (auto i : rec) {
    ...
    for (auto &x : todo) {
        ...
        unsigned uid = 0;
        auto pred = [&bound](Ent const &x, Ent const &y) {
        ...
        while (!open.empty()) {
            ...
            auto index(y->data.lit.index(y->data.type, bound));
...
void BodyAggregateAccumulate::linearize(bool positive) {
    ...
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), lits, &auxLits);
}
void AssignmentAggregateAccumulate::linearize(bool positive) {
    Term::VarSet important;
    defines.collectImportant(important);
    VarTermBoundVec vars;
    for (auto &x : tuple) { x->collect(vars, false); }
...
void ConjunctionAccumulateEmpty::linearize(bool positive) {
    Term::VarSet important;
    conjDef.collectImportant(important);
    ULitVec lits;
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), lits, &auxLits);
}
void ConjunctionAccumulate::linearize(bool positive) {
    Term::VarSet important;
    conjDef.collectImportant(important);
    if (conjDom->head) { 
    ...
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), conjDom->lits, &auxLits);
}
void ConjunctionAccumulateFact::linearize(bool positive) {
    Term::VarSet important;
    conjDef.collectImportant(important);
    conjDom->head->headRep->collect(important);
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), conjDom->lits, &auxLits);
}
void HeadAggregateAccumulate::linearize(bool positive) {
    Term::VarSet important;
    if (headNum > 0) { dom->head(headNum).second.collectImportant(important); }
    for (auto &x : tuple) { x->collect(important); }
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), lits);
}
void DisjunctionAccumulate::linearize(bool positive) {
    Term::VarSet important;
    if (def) { def->collectImportant(important); }
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), lits);
}
void HeadAggregateRule::linearize(bool positive) {
    Term::VarSet important;
    def.collectImportant(important);
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), lits);
}
void DisjunctionRule::linearize(bool positive) {
    Term::VarSet important;
    def.collectImportant(important);
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), lits);
}
void Rule::linearize(bool positive) {
    Term::VarSet important;
    if (defines) { defines->collectImportant(important); }
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), lits);
}
void WeakConstraint::linearize(bool positive) {
    Term::VarSet important;
    for (auto &x : tuple) { x->collect(important); }
    insts = Gringo::Ground::linearize(positive, *this, std::move(important), lits);
}
...
void BodyAggregateAccumulate::print(std::ostream &out) const {
    ...
    print_comma(out, lits.begin(), lits.end(), g, ",");
    out << ":-";
    print_comma(out, auxLits.begin(), auxLits.end(), g, ",");
    out << ".";
}
...
void Rule::print(std::ostream &out) const {
    ...
    if (!lits.empty()) {
        ...
        print_comma(out, lits.begin(), lits.end(), f, ",");
    }
    out << ".";
}
