...
void Defines::init() {
    ...
    for (auto &scc : graph.tarjan()) {
        ...
        for (auto &x : scc) { Term::replace(std::get<2>(x->data->second), std::get<2>(x->data->second)->replace(*this)); }
    }
}

bool Defines::empty() const { return defs_.empty(); }

void Defines::apply(Value x, Value &retVal, UTerm &retTerm) {
    switch (x.type())  {
        case Value::FUNC: {
            ValVec args;
            bool changed = true;
...
void GFunctionTerm::print(std::ostream &out) const {
    out << name;
    out << "(";
    print_comma(out, args.begin(), args.end(), [](std::ostream &out, UGTerm const &x) { out << *x; }, ",");
    out << ")";
}
...
Term::SimplifyRet PoolTerm::simplify(DotsMap &, unsigned &, bool, bool)   { return {*this, false}; }
Term::SimplifyRet ValTerm::simplify(DotsMap &, unsigned &, bool, bool)    { return {value}; }
Term::SimplifyRet LinearTerm::simplify(DotsMap &, unsigned &, bool, bool) { return {*this, false}; }
Term::SimplifyRet VarTerm::simplify(DotsMap &, unsigned &auxNum, bool positional, bool arithmetic) {
    if (name == "_") {
        ref = std::make_shared<Value>();
        if (positional) { return {*this, true}; }
        else { name = Term::uniqueName(auxNum); }
    }
    if (arithmetic) { return {make_locatable<LinearTerm>(loc(), *this, 1, 0)}; }
    else            { return {*this, false}; }
}
Term::SimplifyRet UnOpTerm::simplify(DotsMap &dots, unsigned &auxNum, bool, bool) {
    SimplifyRet ret(arg->simplify(dots, auxNum, false, true));
    if(ret.notNumeric()) {
...
Term::SimplifyRet BinOpTerm::simplify(DotsMap &dots, unsigned &auxNum, bool, bool) {
    auto retLeft(left->simplify(dots, auxNum, false, true));
    auto retRight(right->simplify(dots, auxNum, false, true));
...
Term::SimplifyRet DotsTerm::simplify(DotsMap &dots, unsigned &auxNum, bool, bool) {
...
Term::SimplifyRet LuaTerm::simplify(DotsMap &dots, unsigned &auxNum, bool, bool) {
    bool constant = true;
    for (auto &arg : args) { constant = arg->simplify(dots, auxNum, false, false).update(arg).type == SimplifyRet::CONSTANT && constant; }
    if (constant) { return {eval()}; }
    else          { return {*this, false}; }
}
Term::SimplifyRet FunctionTerm::simplify(DotsMap &dots, unsigned &auxNum, bool positional, bool) {
    bool constant  = true;
    bool projected = false;
    for (auto &arg : args) { 
        auto ret(arg->simplify(dots, auxNum, positional, false));
...
//           never null
Term::ProjectRet PoolTerm::project(unsigned &) {
    throw std::logic_error("Term::project must be called after Term::unpool");
}
Term::ProjectRet ValTerm::project(unsigned &) { 
    return std::make_tuple(nullptr, UTerm(clone()), UTerm(clone()));
}
Term::ProjectRet LinearTerm::project(unsigned &auxNum) {
    UTerm x(uniqueVar(loc(), auxNum).release());
    UTerm y(x->clone());
    return std::make_tuple(nullptr, std::move(x), std::move(y));
}
Term::ProjectRet VarTerm::project(unsigned &auxNum) {
    if (*name == "_") { return std::make_tuple(nullptr, nullptr, uniqueVar(loc(), auxNum, 0, "#P")); }
    else {
        UTerm x(uniqueVar(loc(), auxNum).release());
        UTerm y(x->clone());
        return std::make_tuple(nullptr, std::move(x), std::move(y));
...
Term::ProjectRet UnOpTerm::project(unsigned &auxNum) {
    UTerm x(uniqueVar(loc(), auxNum).release());
    UTerm y(x->clone());
    return std::make_tuple(nullptr, std::move(x), std::move(y));
}
Term::ProjectRet BinOpTerm::project(unsigned &auxNum) {
    UTerm x(uniqueVar(loc(), auxNum).release());
    UTerm y(x->clone());
    return std::make_tuple(nullptr, std::move(x), std::move(y));
}
Term::ProjectRet DotsTerm::project(unsigned &) {
    throw std::logic_error("Term::project must be called after Term::simplify");
}
Term::ProjectRet LuaTerm::project(unsigned &auxNum) {
    UTerm x(uniqueVar(loc(), auxNum).release());
    UTerm y(x->clone());
    return std::make_tuple(nullptr, std::move(x), std::move(y));
}
Term::ProjectRet FunctionTerm::project(unsigned &auxNum) {
    UTermVec argsProjected;
    UTermVec argsProject;
    auto jt = args.begin(), ie = args.end();
    for (auto it = jt; it != ie; ++it) {
        auto ret((*it)->project(auxNum));
        Term::replace(*it, std::move(std::get<0>(ret)));
        if (std::get<1>(ret)) {
    ...
    args.erase(jt, ie);
    UTerm projected;
    if (*name == "" && argsProjected.size() == 1) { projected = std::move(argsProjected.front()); }
    else if (!argsProjected.empty())              { projected = make_locatable<FunctionTerm>(loc(), name, std::move(argsProjected)); }
    else                                          { projected = make_locatable<ValTerm>(loc(), Value(name)); }
    UTerm self;
    if (args.empty())                         { self = make_locatable<ValTerm>(loc(), Value(name)); }
    else if (*name == "" && args.size() == 1) { self = std::move(args.front()); }
    return std::make_tuple(
        std::move(self),  
        std::move(projected), 
        make_locatable<FunctionTerm>(loc(), name, std::move(argsProject)));
}
...
bool LinearTerm::operator==(Term const &x) const {
    ...
    return t && m == t->m && n == t->m && is_value_equal_to(var, t->var);
}
...
UTerm ValTerm::replace(Defines &x, bool replace) {
    if (replace && value.type() == Value::ID) { 
        auto it(x.defs().find(value.string()));
        if (it != x.defs().end()) { return get_clone(std::get<2>(it->second)); }
    }
    return nullptr;
}
...
UTerm LuaTerm::replace(Defines &x, bool) {
    for (auto &y : args) { Term::replace(y, y->replace(x)); }
    return nullptr;
}
...
UTerm Term::insert(ArithmeticsMap &arith, unsigned &auxNum, UTerm &&term) {
    ...
    if (ret.second) { ret.first->second = uniqueVar(ret.first->first->loc(), auxNum, level); }
...
std::unique_ptr<LinearTerm> LinearTerm::create(Location const &loc, DotsMap &dots, unsigned &auxNum, UTerm &&left, UTerm &&right) {
    dots.emplace_back(Term::uniqueVar(loc, auxNum), std::move(left), std::move(right));
