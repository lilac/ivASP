...
void Rule::printPlain(std::ostream &out) const {
    bool isShow = head && head->first.sig() == Signature("#show", 1);
    if (isShow)    { out << "#show " << *head->first.args().begin(); }
...
void Rule::toLparse(unsigned &auxAtoms, StmHandler const &x) {
    for (auto &y : body) { Term::replace(y, y->toLparse(auxAtoms, x)); }
    x(*this);
}
...
void RuleRef::toLparse(unsigned &auxAtoms, StmHandler const &x) {
    ULitVec bd;
    for (auto &y : body) {
        if (ULit z = y.get().toLparse(auxAtoms, x)) {
...
LparseRule::LparseRule(SAuxAtomVec &&head, ULitVec &&body) : choice(false), auxHead(std::move(head)), body(std::move(body)) { }
...
void LparseRule::toLparse(unsigned &auxAtoms, StmHandler const &x) {
    for (auto &y : body) { Term::replace(y, y->toLparse(auxAtoms, x)); }
    x(*this);
}
...
void WeightRule::toLparse(unsigned &auxAtoms, StmHandler const &x) {
    for (auto &y : body) { Term::replace(y.first, y.first->toLparse(auxAtoms, x)); }
    x(*this);
}
...
void HeadAggregateState::accumulate(ValVec const &tuple, AggregateFunction fun, PredicateDomain::element_type *head, unsigned headNum, LitVec const &lits, Location const &loc) {
    ...
    if (!ignore || head) {
        ...
        bool remove(fact && !ret.first->second->fact);
...
void HeadAggregateRule::toLparse(unsigned &auxAtoms, StmHandler const &x) {
    // TODO: this algorithm suffers from severe uglyness!!!
    // TODO: handle empty bounds here...
    using GroupByCond = std::unique_list<std::reference_wrapper<ULitVec>, std::pair<std::shared_ptr<AuxAtom>, std::vector<std::pair<FWValVec, HeadAggregateElement::Cond&>>>, value_hash<ULitVec>, value_equal_to<ULitVec>>;
    ...
    if (!body.empty()) {
        // b :- body.
        b = std::make_shared<AuxAtom>(auxAtoms++);
        LparseRule(b, std::move(body)).toLparse(auxAtoms, x);
    }
    for (auto &y : groupByCond) {
        if (!y.first.get().empty()) { 
            // c :- C.
            y.second->first = std::make_shared<AuxAtom>(auxAtoms++);
            LparseRule(y.second->first, get_clone(y.first.get())).toLparse(auxAtoms, x);
        }
        // { heads } :- c, b.
        ...
        if (!atoms.empty()) {
            ...
            LparseRule(std::move(atoms), std::move(lits), true).toLparse(auxAtoms, x);
        }
    }
        ...
        for (auto &y : groupByCond) {
            for (auto &z : y.second->second) {
                ...
                if (it != tupleAtoms.end()) {
                    LparseRule(it->second, y.second->first ? make_unique<AuxLiteral>(y.second->first, false) : nullptr, nullptr).toLparse(auxAtoms, x);
                }
            }
        }
        switch (fun) {
            ...
            case AggregateFunction::SUM: {
                ...
                for (auto &y : repr->bounds) {
                    ...
                    if (rng.left < y.left) {
                        // a :- L { }.
                        SAuxAtom a(std::make_shared<AuxAtom>(auxAtoms++));
                    ...
                    LparseRule(aggr, std::move(aLits)).toLparse(auxAtoms, x);
                }
                // :- b, not aggr.
                LparseRule(b ? make_unique<AuxLiteral>(b, false) : nullptr, make_unique<AuxLiteral>(aggr, true)).toLparse(auxAtoms, x);
                break;
            }
            case AggregateFunction::MIN:
            case AggregateFunction::MAX: {
                ...
                for (auto &y : repr->bounds) {
                    ...
                    if (fun == AggregateFunction::MIN ? y.right < rng.right : rng.left < y.left) {
                        ...
                        for (auto &z : elemVec) {
                            // a :- 1 { between }
                            if (y.contains(z.second)) { LparseRule(a, get_clone(z.first), nullptr).toLparse(auxAtoms, x); }
                        }
                        aLits.emplace_back(make_unique<AuxLiteral>(a, false));
                    }
                    if (fun == AggregateFunction::MIN ? rng.left < y.left : y.right < rng.right) {
                        ...
                        for (auto &z : elemVec) {
                            if (fun == AggregateFunction::MIN ? z.second < y : y < z.second) {
                                // if min:  a :- 1 { below }
                                // eif max: a :- 1 { above }
                                LparseRule(a, get_clone(z.first), nullptr).toLparse(auxAtoms, x);
                            }
                        }
                        aLits.emplace_back(make_unique<AuxLiteral>(a, true));
                    }
...
void DisjunctionRule::toLparse(unsigned &auxAtoms, StmHandler const &x) {
    SAuxAtom            bdAtom;
    ...
    for (auto &y : repr->elems) {
        if (y.first && y.second.empty()) { djHead.emplace_back(*y.first); }
            ...
            if (y.second.size() == 1) { 
                ...
                SAuxAtom ncdAtom(std::make_shared<AuxAtom>(auxAtoms++));
                LparseRule(ncdAtom, get_clone(cdLit), nullptr).toLparse(auxAtoms, x);
                ncdLit = make_unique<AuxLiteral>(ncdAtom, true);
            }
            else {
                SAuxAtom cdAtom(std::make_shared<AuxAtom>(auxAtoms++));
                LparseRule(cdAtom, std::move(y.second)).toLparse(auxAtoms, x);
                cdLit  = make_unique<AuxLiteral>(cdAtom, false);
                ncdLit = make_unique<AuxLiteral>(cdAtom, true);
            }
            SAuxAtom hdAtom(std::make_shared<AuxAtom>(auxAtoms++));
            if (y.first) {
                // y.first :- hdAtom, cdLit.
                LparseRule(*y.first, std::make_unique<AuxLiteral>(hdAtom, false), get_clone(cdLit)).toLparse(auxAtoms, x);
            }
            // hdAtom  :- y.first, cdLit.
            LparseRule(hdAtom, y.first ? make_unique<PredicateLiteral>(NAF::POS, *y.first) : nullptr, std::move(cdLit)).toLparse(auxAtoms, x);
            // :- hdAtom, not cdLit.
            LparseRule(std::make_unique<AuxLiteral>(hdAtom, false), std::move(ncdLit)).toLparse(auxAtoms, x);
            djAuxHead.emplace_back(hdAtom);
        }
    }
    LparseRule(std::move(djHead), std::move(djAuxHead), std::move(body), false).toLparse(auxAtoms, x);
}
