...
public:
	using InputGen = std::function<std::unique_ptr<Clasp::Input> (Application &app)>;
	...
	int    run(int argc, char** argv, const InputGen ig); // "entry-point"
	void   printTemplate()const;
	void   printWarnings()const;
	std::istream& getStream();
	std::vector<std::string>& getInputs() { return app_.input; }
private:
	Application();
