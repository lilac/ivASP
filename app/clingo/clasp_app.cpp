...
void Application::installSigHandlers() {
	for (int i = 0; i != sizeof(signals)/sizeof(int); ++i) {
		if (signal(signals[i], &Application::sigHandler) == SIG_IGN) {
...
/////////////////////////////////////////////////////////////////////////////////////////
int Application::run(int argc, char** argv, const InputGen ig) {
	...
	installSigHandlers();
	int retStatus = E_UNKNOWN;
	ClaspFacade clasp;
	stats_.clear();
	SolveStats agg;
	try {
		std::unique_ptr<Clasp::Input> inputPtr = std::move(ig(*this));
		Clasp::Input &input = *inputPtr;
		...
		retStatus = printResult(sig);
	}
	catch (const std::bad_alloc&  ) { retStatus = exception(E_MEMORY, "std::bad_alloc"); }
	catch (const std::exception& e) { retStatus = exception(E_ERROR, e.what()); }
...
// for printing.
int Application::printResult(int sig) {
