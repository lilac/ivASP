...
bool SharedDependencyGraph::relevantPrgAtom(const Solver& s, PrgAtom* a) const { 
	return a->inUpper() && a->scc() != PrgNode::noScc && !s.isFalse(a->literal());
}
...
void SharedDependencyGraph::addSccs(ProgramBuilder& prg, const AtomList& sccAtoms, const NonHcfSet& nonHcfs) {
	...
	for (AtomList::size_type i = 0, end = sccAtoms.size(); i != end; ++i) {
		...
		if (relevantPrgAtom(*ctx.master(), a)) { 
			...
			for (PrgAtom::dep_iterator it = a->deps_begin(), endIt = a->deps_end(); it != endIt; ++it) {
				if (!it->sign()) {
					PrgBody* prgBody = prg.getBody(it->var());
...
uint32 SharedDependencyGraph::addBody(const ProgramBuilder& prg, PrgBody* b) {
	if (b->seen()) {     // first time we see this body - 
		...
		b->resetId(bId, false);
		prg.context()->setFrozen(b->var(), true);
	}
	return b->id();
}
...
void SharedDependencyGraph::addPreds(const ProgramBuilder& prg, PrgBody* b, uint32 bScc, VarVec& preds) const {
	if (bScc == PrgNode::noScc) { preds.clear(); return; }
	const bool weights = b->type() == BodyInfo::SUM_BODY;
	for (uint32 i = 0; i != b->size() && !b->goal(i).sign(); ++i) {
		PrgAtom* pred = prg.getAtom(b->goal(i).var());
		if (relevantPrgAtom(*prg.context()->master(), pred) && pred->scc() == bScc) {
			preds.push_back( pred->id() );
			if (weights) { preds.push_back(b->weight(i)); }
		}
	}
	if (b->type() != BodyInfo::NORMAL_BODY) {
		...
		for (uint32 n = 0; n != b->size(); ++n) {
			...
			if (ext && !prg.context()->master()->isFalse(lit)) {
...
uint32 SharedDependencyGraph::addHeads(const ProgramBuilder& prg, PrgBody* b, VarVec& heads) const {
	for (PrgBody::head_iterator it = b->heads_begin(), end = b->heads_end(); it != end; ++it) {
		if (it->isAtom() && !it->isGamma()) {
			PrgAtom* a = prg.getAtom(it->node());
			if (relevantPrgAtom(*prg.context()->master(), a)) {
...
uint32 SharedDependencyGraph::getAtoms(const ProgramBuilder& prg, PrgDisj* d, VarVec& atoms) const {
...
uint32 SharedDependencyGraph::addDisj(const ProgramBuilder& prg, PrgDisj* d) {
	...
	if (d->seen()) { // first time we see this disjunction
		...
		if (relevantPrgBody(*prg.context()->master(), prgBody)) {
...
void SharedDependencyGraph::enableStats(uint32 lev) {
	if (lev > 1) {
		for (ComponentMap::iterator it = components_.begin(), end = components_.end(); it != end; ++it) {
			it->second->enableStats(lev - 1);
		}
	}
}
/////////////////////////////////////////////////////////////////////////////////////////
...
void SharedDependencyGraph::NonHcfComponent::ComponentMap::addAtomConstraints(SharedContext& comp) {
	...
	cc1.addFlags(ClauseCreator::clause_force_simplify);
	cc1.start();
...
void SharedDependencyGraph::NonHcfComponent::ComponentMap::addBodyConstraints(const Solver& generator, const SccGraph& dep, uint32 scc, SharedContext& comp) {
	...
	cc.addFlags(ClauseCreator::clause_force_simplify);
...
/////////////////////////////////////////////////////////////////////////////////////////
SolveTestEvent::SolveTestEvent(const Solver& s, uint32 a_scc, bool part) : SolveEvent(s, SolveEvent_t::progress_test), result(-1), scc(a_scc), partial(part) {
	confDelta   = s.stats.conflicts;
	choiceDelta = s.stats.choices;
	timeDelta   = s.stats.extra ? s.stats.extra->cpuTime : 0.0;
}
...
double SolveTestEvent::time() const {
	return solver->stats.extra ? solver->stats.extra->cpuTime - timeDelta : 0.0;
}
...
void SharedDependencyGraph::NonHcfComponent::enableStats(uint32 lev) {
	if (lev != 0) { prg_->enableStats(lev); }
}
...
bool SharedDependencyGraph::NonHcfComponent::test(uint32 scc, const Solver& generator, const LitVec& assume, VarVec& unfoundedOut) const {
	...
	static SolveAlgorithm def;
	SolveAlgorithm* algo = generator.sharedContext()->solve;
	if (!algo) { algo    = &def; }
	Timer<ThreadTime> t;
	uint32 sId  = generator.id();
	Solver& s   = *prg_->solver(sId);
	SolveTestEvent ev(s, scc, generator.numFreeVars() != 0);
	if (s.stats.extra) { ev.timeDelta = s.stats.extra->cpuTime; t.start(); s.stats.addTest(ev.partial); }
	generator.sharedContext()->reportProgress(ev);
	bool sat    = algo->satisfiable(*prg_, s, prg_->configuration()->search(sId), assume);
	ev.result   = !sat;
	if (sat) {
		s.stats.addModel(s.decisionLevel());
		comp_->mapTesterModel(s, unfoundedOut); 
	}
	if (s.stats.extra) { t.stop(); s.stats.extra->cpuTime += t.total(); }
	generator.sharedContext()->reportProgress(ev);
	return !sat;
}
}
