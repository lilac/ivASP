...
bool Preprocessor::preprocessSimple() {
	...
	uint32 startVar = prg_->context()->numVars() + 1;
...
bool Preprocessor::preprocessEq(uint32 maxIters) {
	...
	Heads    heads  = { 
		{prg_->disj_begin(), prg_->disj_end()}, 
		{prg_->atom_begin()+prg_->startAtom(), prg_->atom_end()} 
	};
	pass_           = 0;
	maxPass_        = maxIters;
	...
	do {
		if (++pass_ > 1) {
			for (uint32 i = 0; i != sizeof(heads)/sizeof(heads[0]); ++i) {
				for (ProgramBuilder::HeadIter it = heads[i][0]; it != heads[i][1]; ++it) {
					(*it)->clearLiteral(false);
					(*it)->setInUpper(false);
				}
			}
			prg_->context()->popVars(startVar);
			litToNode_.clear();
		}
		VarVec& supported = prg_->getSupportedBodies(true);
		if (!classifyProgram(supported)) { return false; }
		res = simplifyClassifiedProgram(heads, pass_ != maxPass_, supported);
...
ValueRep Preprocessor::simplifyClassifiedProgram(const Heads& heads, bool more, VarVec& supported) {
	...
	if (!prg_->propagate()) { return value_false; }
	for (ProgramBuilder::VarIter it = prg_->unfreeze_begin(), end = prg_->unfreeze_end(); it != end; ++it) {
...
bool Preprocessor::addHeadsToUpper(PrgBody* body) {
	...
	for (PrgBody::head_iterator it = body->heads_begin(), end = body->heads_end(); it != end && ok; ++it) {
		...
		if (head->relevant()) {
			...
			else if (head->supps_begin()->isNormal()) {
...
ValueRep Preprocessor::simplifyBody(PrgBody* b, bool reclass, VarVec& supported) {
	...
	if (b->relevant() && eqId != b->id() && (reclass || prg_->getBody(eqId)->var() == b->var())) {
		// Body is now eq to some other body - reclassify only if body var is not needed
		PrgBody* eqBody = prg_->mergeEqBodies(b, eqId, true, true);
		if (eqBody && eqBody->var() != b->var() && hasRootLiteral(b)) {
			ret = value_weak_true;
		}
	}
