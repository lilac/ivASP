...
/////////////////////////////////////////////////////////////////////////////////////////
Var PrgRuleTransform::AdaptBuilder::newAtom() {
	return prg_->newAtom();
}
void PrgRuleTransform::AdaptBuilder::addRule(PrgRule& rule) {
	prg_->addRule(rule);
}

class PrgRuleTransform::Impl {
public:
	Impl(ProgramAdapter& prg, PrgRule& r);
	~Impl();
	uint32 transform();
private:
	Impl(const Impl&);
...
uint32 PrgRuleTransform::transform(ProgramAdapter& prg, PrgRule& rule) {
...
PrgRuleTransform::Impl::Impl(ProgramAdapter& prg, PrgRule& r)
	: prg_(prg)
	, rule_(r) {
...
void PrgRuleTransform::Impl::createRule(Var head, Literal* bodyFirst, Literal* bodyEnd) {
	out_.clear();
...
// h :- b, d.
// h :- c, d.
uint32 PrgRuleTransform::transformNoAux(ProgramAdapter& prg, PrgRule& rule) {
...
// auxB :- BODY.
uint32 PrgRuleTransform::transformChoiceRule(ProgramAdapter& prg, PrgRule& rule) const {
	uint32 newRules = 0;
...
// auxB :- BODY.
uint32 PrgRuleTransform::transformDisjunctiveRule(ProgramAdapter& prg, PrgRule& rule) const {
	uint32 newRules = 0;
	PrgRule temp; temp.setType(BASICRULE);
	if (rule.body.size() > 1) {
		PrgRule bodyR;
...
// SCC/cycle checking
/////////////////////////////////////////////////////////////////////////////////////////
PrgSccChecker::PrgSccChecker(ProgramBuilder& prg, AtomList& sccAtoms, uint32 startScc)
...
void PrgSccChecker::visitDfs(PrgNode* node, NodeType t) {
...
bool PrgSccChecker::recurse(Call& c) {
...
bool PrgSccChecker::onNode(PrgNode* n, NodeType t, Call& c, uint32 data) {
	if (!n->seen()) {
...
PrgHead::PrgHead(uint32 id, NodeType t, uint32 data, bool checkScc)
	: PrgNode(id, checkScc)
	, data_(data), upper_(0), dirty_(0), disj_(0), frozen_(0), isAtom_(t == PrgEdge::ATOM_NODE)  {
...
bool PrgHead::simplifySupports(ProgramBuilder& prg, bool strong, uint32* numDiffSupps) {
	...
	if (dirty_ == 1) {
		...
		disj_    = static_cast<uint32>(dis > 0);
		numLits += choices;
	}
...
void PrgHead::assignVar(ProgramBuilder& prg, PrgEdge support) {
	...
	if (numB == 0 && support == PrgEdge::noEdge()) {
		...
		if (support.isNormal() && (numB == 1 || sup->value() == value_true)) {
			...
			prg.context()->setVarEq(var(), true);
			prg.incEqs(Var_t::atom_body_var);
		}
		else {
			setLiteral(posLit(prg.context()->addVar(Var_t::atom_var)));
		}
	}
}

// Backpropagates the value of this head to its supports.
bool PrgHead::backpropagate(ProgramBuilder& prg, ValueRep val, bool bpFull) {
	bool ok = true;
	if (val == value_false) {
...
void PrgAtom::removeDep(Var bodyId, bool pos) {
	LitVec::iterator it = std::remove(deps_.begin(), deps_.end(), Literal(bodyId, !pos));
	deps_.erase(it, deps_.end());
}
...
bool PrgAtom::propagateValue(ProgramBuilder& prg, bool backprop) {
	ValueRep val = value();
	assert(val != value_free);
	// propagate value forward
	for (dep_iterator it = deps_.begin(), end = deps_end(); it != end; ++it) {
		if (!prg.getBody(it->var())->propagateAssigned(prg, *it, val)) {
			return false;
		}
	}
	if (prg.isFact(this) && disj_) {
...
bool PrgAtom::addConstraints(const ProgramBuilder& prg, ClauseCreator& gc) {
	SharedContext& ctx = *prg.context();
	...
	for (EdgeVec::iterator it = supports_.begin(); it != supports_.end(); ++it) {
		...
		if (n->relevant() && n->hasVar()) {
			*j++ = *it;
			...
			if (!ctx.marked(B) && !ctx.master()->isFalse(B) && !it->isDisj()) {
				ctx.mark( B );
				gc.add(B);
				sat |= ctx.marked( ~B );
			}
	...
	supports_.erase(j, supports_.end());
	ctx.unmark( var() );
	for (LitVec::size_type i = 0; i != gc.size(); ++i) {
		ctx.unmark(gc[i].var());
	}
...
// class PrgBody
/////////////////////////////////////////////////////////////////////////////////////////
PrgBody::PrgBody(ProgramBuilder& prg, uint32 id, const BodyInfo& body, bool addDeps)
	: PrgNode(id, true)
...
PrgBody* PrgBody::create(ProgramBuilder& prg, uint32 id, const BodyInfo& body, bool addDeps) {
...
uint32 PrgBody::findLit(const ProgramBuilder& prg, Literal p) const {
...
bool PrgBody::simplifyBody(ProgramBuilder& prg, bool strong, uint32* eqId) {
	if (eqId)        { *eqId  = id(); }
	...
	for (Literal* it = j, *end = j + size(); it != end; ++it) {
		...
		if (addEq) {
			...
			*it   = prg.getAtom(a)->eqGoal(it->sign());   // replace with eq goal
			a     = it->var();                            // and remember to add the new edge later
		}
		...
		if (v == value_true || v == value_false) { // truth value is known - remove subgoal
			...
			*j++    = *it;       // copy literal
			...
			if (type() != BodyInfo::NORMAL_BODY) { // merge subgoal
				...
				// ignore if normal
				--bound;
				if (!addEq) prg.getAtom(it->var())->removeDep(id(), !it->sign());
			} 
		}
	}
	...
	bool ok = normalize(bound, sumW, reachW, newHash);
	if (ok) {
		Var xId = id();
		if (oldHash != newHash) {
...
bool PrgBody::normalize(weight_t bound, weight_t sumW, weight_t reachW, uint32& hashOut) {
...
// any duplicate heads.
void PrgBody::prepareSimplifyHeads(ProgramBuilder& prg, RuleState& rs) {
...
bool PrgBody::simplifyHeadsImpl(ProgramBuilder& prg, RuleState& rs, bool strong) {
	ProgramBuilder::EdgeRange r;
	PrgHead* cur;
	PrgEdge* j     = const_cast<PrgEdge*>(heads_begin());
	uint32 newSize = 0;
	bool block     = value() == value_false;
	for (head_iterator it = heads_begin(), end = heads_end(); it != end; ++it) {
		cur  = prg.getHead(*it);
		block= block || blockedHead(*it, rs);
		if (!cur->relevant() || (strong && !cur->hasVar()) 
			|| block || superfluousHead(prg, cur, *it, rs)) {
...
bool PrgBody::simplifyHeads(ProgramBuilder& prg, bool strong) {
	if (sHead_ == 0) { return true; }
	return PrgBody::mergeHeads(prg, *this, strong);
}

bool PrgBody::mergeHeads(ProgramBuilder& prg, PrgBody& heads, bool strong) {
	...
	// 2. remove duplicate/superfluous atoms & check for blocked atoms
	prepareSimplifyHeads(prg, rs);
	if (this != &heads) { heads.prepareSimplifyHeads(prg, rs); }
	bool ok = simplifyHeadsImpl(prg, rs, strong);
	if (this != &heads) {
		// 3. simplify heads to merge...
		ok     = heads.simplifyHeadsImpl(prg, rs, strong) && ok;
		// ...and copy new heads
		for (head_iterator it = heads.heads_begin(), end = heads.heads_end(); it != end; ++it) {
			addHead(prg.getHead(*it), it->type());
		}
	}
	// 3. clear temporary flags & reestablish ordering
...
bool PrgBody::superfluousHead(const ProgramBuilder& prg, const PrgHead* head, PrgEdge it, const RuleState& rs) const {
	if (it.isAtom()) {
		// the head is an atom
		...
		if (prg.options().iters == UINT32_MAX) {
			for (head_iterator it = heads_begin(), end = heads_end(); it != end; ++it) {
...
void PrgBody::assignVar(ProgramBuilder& prg) {
	if (hasVar() || !relevant()) { return; }
	...
	if (size == 0 || value() == value_true) {
		...
		setLiteral(posLit(prg.context()->addVar(Var_t::body_var)));	
	}
	else {
		setLiteral(negLit(0));
	}
...
bool PrgBody::eqLits(WeightLitVec& vec, bool& sorted) const {
	if (!sorted && vec.size() <= 10) {
		for (WeightLitVec::const_iterator it = vec.begin(), end = vec.end(); it != end; ++it) {
			...
			if (x == goals_end() || weight(std::distance(goals_begin(), x)) != it->second) {
				return false;
			}
		}
	}
...
bool PrgBody::propagateSupported(Var v) {
	weight_t w = 1;
	if (hasWeights()) {
		uint32 pos = std::distance(goals_begin(), std::find(goals_begin(), goals_end(), posLit(v)));
		w          = weight(pos);
	}
	return (unsupp_ -= w) <= 0;
}

bool PrgBody::propagateAssigned(ProgramBuilder& prg, Literal p, ValueRep v) {
...
bool PrgBody::propagateAssigned(ProgramBuilder& prg, PrgHead* h, EdgeType t) {
	if (!relevant()) return true;
	markHeadsDirty();
	if (h->value() == value_false) {
		if (t == PrgEdge::NORMAL_EDGE && value() != value_false) {
			return assignValue(value_false) && propagateValue(prg, prg.options().backprop);
		}
		eraseHead(PrgEdge::newEdge(h->id(), t, h->isAtom() ? PrgEdge::ATOM_NODE : PrgEdge::DISJ_NODE));
	}
	return true;
}

bool PrgBody::propagateValue(ProgramBuilder& prg, bool backprop) {
	ValueRep val = value();
...
bool PrgBody::addConstraints(const ProgramBuilder& prg, ClauseCreator& gc) {
	SharedContext& ctx = *prg.context();
	Literal eq;
	if (type() == BodyInfo::NORMAL_BODY) {
		...
		for (const Literal* it = goals_begin(), *end = goals_end(); it != end; ++it) {
			assert(it->var() != 0);
			eq = prg.getAtom(it->var())->literal() ^ it->sign();
			if (negBody != ~eq) {
				bc.start().add(negBody);
				if ( (negBody != eq && !bc.add(eq).end()) || (negBody == eq && !bc.end()) ) {
					return false;
				}
			} // else: SAT-clause - ~b b
			sat |= eq == literal();
		...
		for (uint32 i = 0, end = size_; i != end; ++i) {
			eq = prg.getAtom(goal(i).var())->literal() ^ goal(i).sign();
...
// - noScc otherwise
uint32 PrgBody::scc(const ProgramBuilder& prg) const {
	uint64 sccMask = 0;
	uint32 end     = size();
...
void PrgDisj::detach(ProgramBuilder& prg) {
...
bool PrgDisj::propagateAssigned(ProgramBuilder& prg, PrgHead* head, EdgeType t) {
