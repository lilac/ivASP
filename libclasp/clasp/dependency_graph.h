   ...
   * The predecessors of a BodyNode are the body's subgoals.
	...
	struct BodyNode : public Node {
		...
		explicit BodyNode(PrgBody* b, uint32 scc) : Node(b->literal(), scc) {
			if (scc == PrgNode::noScc || b->type() ==  BodyInfo::NORMAL_BODY) {
				data = type_normal;
			}
			else if (b->type() == BodyInfo::COUNT_BODY) { data = type_count; }
			else if (b->type() == BodyInfo::SUM_BODY)   { data = type_sum;   }
	 ...
	 */
	void addSccs(ProgramBuilder& prg, const AtomList& sccAtoms, const NonHcfSet& nonHcfs);
...
private:
	...
	NodeId           addBody(const ProgramBuilder& prg, PrgBody*);
	NodeId           addDisj(const ProgramBuilder& prg, PrgDisj*);
	uint32           addHeads(const ProgramBuilder& prg, PrgBody*, VarVec& atoms) const;
	uint32           getAtoms(const ProgramBuilder& prg, PrgDisj*, VarVec& atoms) const;
	void             addPreds(const ProgramBuilder& prg, PrgBody*, uint32 bScc, VarVec& preds) const;
