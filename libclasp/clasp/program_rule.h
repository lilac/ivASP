...
public:
	...
	virtual void addRule(PrgRule& rule) = 0;
	protected: ~ProgramAdapter() {}
	};
	struct AdaptBuilder : public ProgramAdapter {
		AdaptBuilder(ProgramBuilder& p) : prg_(&p) {}
		Var newAtom();
		void addRule(PrgRule& rule);
		ProgramBuilder* prg_;
	};
	PrgRuleTransform();
	~PrgRuleTransform();
	...
	uint32 transform(ProgramAdapter& prg, PrgRule& rule);
	uint32 transformNoAux(ProgramAdapter& prg, PrgRule& rule);
...
private:
	...
	uint32 transformChoiceRule(ProgramAdapter& prg, PrgRule& rule) const;
	uint32 transformDisjunctiveRule(ProgramAdapter& prg, PrgRule& rule) const;
	class Impl;
	Impl* impl_;
};
...
public:
	...
	bool         simplifySupports(ProgramBuilder& prg, bool strong, uint32* numDiffSupps = 0);
	...
	void         assignVar(ProgramBuilder& prg, PrgEdge it);	
	//@}
protected:
	//! Creates a new node that corresponds to a literal that is false.
	explicit PrgHead(uint32 id, NodeType t, uint32 data = 0, bool checkScc = true);
	bool      backpropagate(ProgramBuilder& prg, ValueRep val, bool bpFull);
	...
	uint32 disj_  :  1; // appears in disjunctive head?
...
public:
	...
	bool         propagateValue(ProgramBuilder& prg, bool backprop);
	bool         addConstraints(const ProgramBuilder& prg, ClauseCreator& c);
...
public:
	 ...
	 */
	static PrgBody* create(ProgramBuilder& prg, uint32 id, const BodyInfo& info, bool addDeps);

	//! Destroys a body node created via create(ProgramBuilder& prg, uint32 id, const PrgRule& rule, const PrgRule::RData& rInfo).
	void     destroy();
	 ...
	 */
	bool     simplifyHeads(ProgramBuilder& prg, bool strong);
	bool     mergeHeads(ProgramBuilder& prg, PrgBody& heads, bool strong);
	 ...
	 */
	bool     simplifyBody(ProgramBuilder& prg, bool strong, uint32* eqId = 0);
	//! Calls simplifyBody() and/or simplifyHeads() if necessary.
	bool     simplify(ProgramBuilder& prg, bool strong, uint32* eqId = 0) {
	...
	bool     propagateAssigned(ProgramBuilder& prg, Literal p, ValueRep v);
	//! Propagates the assignment of a head.
	bool     propagateAssigned(ProgramBuilder& prg, PrgHead* h, EdgeType t);
	...
	void     clearHeads();
	bool     resetSupported();
	void     assignVar(ProgramBuilder& prg);
	bool     assignValue(ValueRep v) { return assignValueImpl(v, noScc()); }
	uint32   scc(const ProgramBuilder& prg) const;
private:
	static const uint32 maxSize = (1u<<26)-1;
	explicit PrgBody(ProgramBuilder& prg, uint32 id, const BodyInfo& info, bool addDep);
	~PrgBody();
	uint32         findLit(const ProgramBuilder& prg, Literal p) const;
	bool           normalize(weight_t bound, weight_t sumW, weight_t reachW, uint32& hashOut);
	void           prepareSimplifyHeads(ProgramBuilder& prg, RuleState& rs);
	bool           simplifyHeadsImpl(ProgramBuilder& prg, RuleState& rs, bool strong);
	bool           superfluousHead(const ProgramBuilder& prg, const PrgHead* head, PrgEdge it, const RuleState& rs) const;
...
public:
	...
	void          destroy();
	void          detach(ProgramBuilder& prg);
	...
	bool          propagateAssigned(ProgramBuilder& prg, PrgHead* at, EdgeType t);
private:
...
class PrgSccChecker {
public:
	PrgSccChecker(ProgramBuilder& prg, AtomList& sccAtoms, uint32 startScc);
